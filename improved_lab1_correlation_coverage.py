...
def path_sim(token1,token2):
    ...
    return round(synset1.path_similarity(synset2), 5)

def leacho_sim(token1,token2):
    synset1 = get_synset(token1)
    synset2 = get_synset(token2)
    return round(synset1.lch_similarity(synset2), 5)

def wupalm_sim(token1,token2):
    synset1 = get_synset(token1)
    synset2 = get_synset(token2)
    return round(synset1.wup_similarity(synset2), 5)
...
def res_sim(token1,token2,ic):
    synset1 = get_synset(token1)
    synset2 = get_synset(token2)
    return round(synset1.res_similarity(synset2, ic), 5)

def jiangcon_sim(token1,token2,ic):
    synset1 = get_synset(token1)
    synset2 = get_synset(token2)
    return round(synset1.jcn_similarity(synset2, ic), 5)

def lin_sim(token1,token2,ic):
    synset1 = get_synset(token1)
    synset2 = get_synset(token2)
    return round(synset1.lin_similarity(synset2,ic),5)

#make empty lists to populate with similarity scores
path_sim_scores = []
leacho_scores = []
wupalm_scores = []
res_scores = []
...
#loop over the word pairs for each sim method
...
for score in generated_scores:
    ...
    print(f"""Spearman's rho for human judgements and {score} similarity is {round(spearman[0],4)} 
